...
const Depoimentos = () => {
  ...
  return <section ref={sectionRef} className="bg-gray-50 py-0">
      <div className="container-section">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {depoimentos.map((depoimento, index) => <motion.div key={index} className="card p-6 bg-white" initial={{
          opacity: 0,
          y: 20
        }} animate={isVisible ? {
          opacity: 1,
          ...
          delay: 0.1 * index
              ...
              <div className="flex items-center mb-4">
                <div className="flex space-x-1">
                  {[...Array(depoimento.stars)].map((_, i) => <Star key={i} className="h-5 w-5 text-yellow-400 fill-yellow-400" />)}
                </div>
              </div>
